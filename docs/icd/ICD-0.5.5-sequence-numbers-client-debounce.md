...
| S.07 | Cursor catches up after writer crash | Seed baseline event/cursor B; a victim process uses the production `events_writer`, commits event V, then is SIGKILLed before broker dispatch; a surviving production writer commits later event S; reconnect replays from B | Yes | Before replay, B < V < S and the durable cursor remains B. Restart/replay emits V then S in order. A later live delivery L has L > S and advances the durable cursor to L. Every child is reaped and task-owned rows are removed. |
