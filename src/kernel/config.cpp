...
auto apply_ws(Config& cfg, const nlohmann::json& j) -> void {
  const bool has_browser_origin = j.contains("browser_origin");
  const bool has_legacy_origin = j.contains("ws_browser_origin");
  const auto browser_origin =
      has_browser_origin ? j["browser_origin"].get<std::string>() : "";
  const auto legacy_origin =
      has_legacy_origin ? j["ws_browser_origin"].get<std::string>() : "";
  if (has_browser_origin && !valid_browser_origin(browser_origin)) {
    throw std::runtime_error("config.browser_origin_invalid");
  }
  if (has_legacy_origin && !valid_browser_origin(legacy_origin)) {
    throw std::runtime_error("config.ws_browser_origin_invalid");
  }
  if (has_browser_origin && has_legacy_origin &&
      browser_origin != legacy_origin) {
    throw std::runtime_error("config.browser_origin_conflict");
  }
  cfg.browser_origin = has_browser_origin ? browser_origin : legacy_origin;
  cfg.ws_browser_origin = cfg.browser_origin;
